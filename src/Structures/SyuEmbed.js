...
module.exports = class SyuEmbed extends MessageEmbed {
	...
	splitFields(contentOrTitle, rawContent) {
		...
		while (content.length) {
			if (content.length < 1024) {
				this.fields.push({ name: this.title, value: content, inline: false });
				return this;
			}
			...
			this.fields.push({ name: this.title, value: content.trim().slice(0, x), inline: false });
